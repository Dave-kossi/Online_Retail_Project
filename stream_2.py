...
# Sélecteur interactif pour carte
option = st.radio(
...
# === 6. Top 10 Produits par pays ===
st.subheader("🛒 Top 10 produits par pays")

# Sélecteur pays
selected_country = st.selectbox("Sélectionner un pays :", country_df["Country"].unique())

top_products_country = ventes[ventes["Country"] == selected_country] \
    .groupby("Description")["Revenue"].sum().reset_index() \
    .sort_values("Revenue", ascending=False).head(10)

fig_top_products = px.bar(
    top_products_country,
    x="Description", y="Revenue",
    title=f"Top 10 produits pour {selected_country}",
    text_auto=".2s",
    color="Revenue"
)
